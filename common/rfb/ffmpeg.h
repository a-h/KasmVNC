...
#define DEFINE_GUARD(name, type) using name##Guard = std::unique_ptr<type, type##Deleter>;
    ...
    using avformat_open_input_func = int (*)(AVFormatContext **ps, const char *url, const AVInputFormat *fmt, AVDictionary **options);
    ...
    using sws_getContext_func = SwsContext *(*) (int srcW, int srcH, AVPixelFormat srcFormat, int dstW, int dstH, AVPixelFormat dstFormat,
                                                 int flags, SwsFilter *srcFilter, SwsFilter *dstFilter, const double *param);

    using sws_scale_func = int (*)(SwsContext *c, const uint8_t *const srcSlice[], const int srcStride[], int srcSliceY, int srcSliceH,
...
public:
    ...
    [[nodiscard]] int avformat_open_input(AVFormatContext **ps, const char *url, const AVInputFormat *fmt, AVDictionary **options) const {
    ...
    [[nodiscard]] int av_hwdevice_ctx_create(AVBufferRef **device_ctx, AVHWDeviceType type, const char *device, AVDictionary *opts,
                                             int flags) const {
    ...
    [[nodiscard]] SwsContext *sws_getContext(int srcW, int srcH, AVPixelFormat srcFormat, int dstW, int dstH, AVPixelFormat dstFormat,
                                             int flags, SwsFilter *srcFilter, SwsFilter *dstFilter, const double *param) const {
        return sws_getContext_f(srcW, srcH, srcFormat, dstW, dstH, dstFormat, flags, srcFilter, dstFilter, param);
    }

    [[nodiscard]] int sws_scale(SwsContext *c, const uint8_t *const src_slice[], const int src_stride[], int src_slice_y, int src_slice_h,
                                uint8_t *const dst[], const int dst_stride[]) const {
